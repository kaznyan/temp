...
class LocalModel(Model):
    ...
    def push_to_global_model(self, batch, global_model, global_optimizer):
        ...
        ### 1エピソード終了時にTD誤差を計算する場合は、ちゃんとreturnを計算できるのでそれ基準で計算する
        ...
        ### 「global_optimizer が学習する」ために細工が必要
        global_optimizer.zero_grad()
        loss.backward() ### ここlossはあくまで local_optimizer から出したものなので
        ### local_optimizer を用いて計算できる grad を global にそのまま与えるとということを書いてやる
        for lp, gp in zip(self.parameters(), global_model.parameters()):
            gp._grad = lp.grad
        ### step して global の学習完了
