...
class Worker(mp.Process):
    ...
    def record(self, score, loss):
        ...
        if self.global_ep.value % log_interval == 0:
            print('{} , {} episode | score: {:.2f}'.format(self.name, self.global_ep.value, self.global_ep_r.value))
    ...
    def run(self):
        ...
        while self.global_ep.value < max_episode:
            ### 各エピソードごとに local_model を global_model に更新
            ...
            while True:
                ...
                if len(memory) == n_step or done:
                    batch = memory.sample() ### このWorkerが1episodeで得た軌跡の全体
                    ### push というより global にこのworkerの経験を「学習」させている
