...
def main():
    ...
    df = pd.DataFrame(columns=["steps", "loss_policy", "loss_value"])
    ...
    for e in range(10000):
        ...
        if e % 16 == 0:
            ### 16ステップごとに、まとめて学習
            loss, loss_policy, loss_value = QNet.train_model(net, optimizer, memory_list)
            ### メモリの初期化
            memory = Memory()
            ...
            print("Ep {0:04d}: score: {1:02d}, loss_policy: {2}, loss_value: {3}".format(e, int(running_score), loss_policy, loss_value))
