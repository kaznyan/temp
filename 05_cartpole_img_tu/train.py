...
def img_to_tensor(img):
    ...
    _img = np.empty((1, 3, img.shape[0], img.shape[1]))
    _img[0, 0] = img[:, :, 0]
    _img[0, 1] = img[:, :, 1]
    _img[0, 2] = img[:, :, 2]
    img_tensor = torch.from_numpy(_img)
    return img_tensor
...
# NN設定
img = render_cv2img(state[0], state[2])
...
for i_episode in range(num_episodes):
    ...
    _state = env.reset()
    last_screen = render_cv2img(_state[0], _state[2])
    last_screen = img_to_tensor(last_screen)
    current_screen = copy.deepcopy(last_screen)
    ...
    for t in count():
        ...
        current_screen = render_cv2img(_state[0], _state[2])
        current_screen = img_to_tensor(current_screen)
        if not done:
        ...
        if done:
            ...
            print("ep:{0}: {1} steps".format(i_episode, t))
            # plot_durations()
            break
